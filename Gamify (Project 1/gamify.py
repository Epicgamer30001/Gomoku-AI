def initialize():
    ...
    global running, resting, textbooks
...
def calc_health():
    global health, time_during, textbooks, running_time_in_a_row, running
    if running == True:
...
def calc_hedons():
    global hedons, time_during, star_run, star_text, tired, bored
    global running, textbooks, running_time_in_a_row, textbooks_time_in_a_row

    if running == True:
        prev_run = running_time_in_a_row - time_during
        not_tired = 0
        if prev_run < 10:
        ...
        bonus = 0
        if not bored == True and star_can_be_taken("running"):
            if time_during <= 10:
                bonus = 3 * time_during
            else:
                bonus = 3 * 10
        ...
        bonus_t = 0
        if not bored == True and star_can_be_taken("textbooks"):
            if time_during <= 10:
